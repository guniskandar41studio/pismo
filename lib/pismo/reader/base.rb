    ...
    class Base
      ...
      def build_doc
        ...
        @doc.css("div, span, table, tr, td, pre").each do |el|
          ...
          el.name = "p" if el.name == "span" && !el.path.tidy_bytes.scan(/[a-z]+/).include?('p')

          el.remove if (FATAL_WORDS & (el['id'].to_s + ' ' + el['class'].to_s).tidy_bytes.downcase.strip.scan(/[a-z]+/)).size > 0
        end
